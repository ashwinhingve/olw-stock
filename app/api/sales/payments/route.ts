...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.amount || !body.customerName || !body.paymentMethod) {
      return NextResponse.json(
        { error: 'Amount, customer name, and payment method are required' },
        { status: 400 }
      );
    }
    ...
    if (!body.reference) {
      body.reference = await Payment.generateReference();
    }
    ...
    if (body.invoice) {
      ...
      // Update invoice payment status (assumes invoice model has paymentMade field)
      const newPaymentTotal = (invoice.paymentMade || 0) + body.amount;
      ...
      await Invoice.findByIdAndUpdate(
        ...
        { 
          $set: { 
            ...
            // If payment covers the invoice, mark as paid
            status: newPaymentTotal >= invoice.total ? 'Paid' : 'Partially Paid'
          } 
        }
      );
    }
